...
from habits.services import get_chat_id, send_tg_message
...
def tg_reminder():
    ...
    reminder = timezone.now() + timedelta(
        minutes=1
    )  # интервал в 1 минуту для устранения погрешности из секунд
    ...
    habits_by_date = [
        habit
        for habit in habits
        if not (current_date - habit.created_date)
    ...
    habits_by_time = [
        habit
        for habit in habits_by_date
        if timezone.now().time() <= habit.time <= reminder.time()
    ]
