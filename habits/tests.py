...
from habits.validators import (
    IsGoodValidate, RewardValidate, validate_periodicity,
...
class HabitTestCase(APITestCase):
    def setUp(self):
        ...
        self.habit0 = Habit.objects.create(
            action="Связанная приятная привычка за выполнение", is_good=True
        )
    ...
    def test_habit_update(self):
        ...
        url = reverse("habits:habit-detail", args=(self.habit.pk,))
        data = {"action": "Новое действие, которое надо сделать"}
        response = self.client.patch(url, data)
        data = response.json()
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            data.get("action"), "Новое действие, которое надо сделать"
        )
            ...
            "results": [
                {
                    ...
                    "time_to_complete": self.habit.time_to_complete,
                    "is_good": False,
                    "related_habit": {"habit": str(self.habit0)},
                    "award": None,
                }
            ],
        }
...
class ValidatorTest(APITestCase):
    ...
    def test_invalid_periodicity_too_low(self):
        ...
        self.assertEqual(
            ...
            [
                ErrorDetail(
                    "Частота выполнения привычки не может быть меньше 1 раз в 7 дней.",
                    code="invalid",
                )
            ],
        )
